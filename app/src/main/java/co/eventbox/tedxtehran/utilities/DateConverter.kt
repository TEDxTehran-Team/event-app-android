...
class DateConverter {
    ...
    constructor(date: String) {
        ...
        setGregorianDate(dates[0].toInt(), dates[1].toInt(), dates[2].toInt())
    }
     ...
     * @return String
     */
    fun getIranianDate(): String {
        val month = if (irMonth >= 10) irMonth.toString() else "0$irMonth"
        val day = if (irDay >= 10) irDay.toString() else "0$irDay"
     ...
     */
    fun setIranianDate(date: String): DateConverter {
        irYear = date.split("/".toRegex()).toTypedArray()[0].toInt()
        irMonth = date.split("/".toRegex()).toTypedArray()[1].toInt()
        irDay = date.split("/".toRegex()).toTypedArray()[2].toInt()
     ...
     * @param day   int
     */
    fun setGregorianDate(year: Int, month: Int, day: Int): DateConverter {
        gYear = year
        gMonth = month
        gDay = day
